...
export const validateFormField = (inputField: any) => {
    let isValid = true;
...
const validateFormat = (input:any) => {
    ...
    if (input !== undefined) {
        ...
        if (input.value !== "" && format !== "" && (format as any).test(input.value) === false) {
            isFormatValid = false;
        }
    }
    return isFormatValid;
}
export const errorClass = (errors: any, fieldName: string) => {
    if (errors !== undefined) {
